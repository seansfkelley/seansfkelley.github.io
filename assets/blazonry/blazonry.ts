...
function bend({ tincture, cotised, ornament }: Ordinary) {
  ...
  if (cotised != null) {
    ...
    bend.appendChild(
      svg.line([0, -offset], [BEND_LENGTH, -offset], cotised, {
        width: COTISED_WIDTH,
      })
    );
    bend.appendChild(
      ...
      for (const sign of [-1, 1]) {
        chevron.appendChild(
          svg.line(
            ...
            cotised,
            { width: COTISED_WIDTH, linecap: "square" }
          )
        );
      }
    }
...
function pale({ tincture, cotised, ornament }: Ordinary) {
  ...
  if (cotised != null) {
    ...
    pale.appendChild(
      svg.line([offset, -H_2], [offset, H_2], cotised, { width: COTISED_WIDTH })
    );
    pale.appendChild(
      svg.line([-offset, -H_2], [-offset, H_2], cotised, {
        width: COTISED_WIDTH,
      })
    );
  }
...
function complexContent(container: SVGElement, content: ComplexContent) {
  ...
  if ("party" in content) {
    ...
    let line = svg.line([0, -H_2], [0, H_2], Tincture.of("sable"), {
      width: 0.5,
    });
    line.setAttribute("vector-effect", "non-scaling-stroke");
    container.appendChild(line);
    line = svg.line([-W_2, 0], [W_2, 0], Tincture.of("sable"), { width: 0.5 });
    line.setAttribute("vector-effect", "non-scaling-stroke");
    container.appendChild(line);
